...
### TrackNet imports
from utils.tracknet.general import *
from utils.tracknet.dataset import Shuttlecock_Trajectory_Dataset, Video_IterableDataset
from utils.tracknet.test import predict_location, get_ensemble_weight, generate_inpaint_mask

### ByteTrack imports
import cv2
from utils.bytetrack import PlayerTracker, read_video, save_video
...
if __name__ == '__main__':
    ...
    out_video_file = os.path.join(args.save_dir, f'{video_name}_shuttle_tracked.mp4')
    out_combined_video_file = os.path.join(args.save_dir, f'{video_name}_player_shuttle_tracked.mp4')
    ...
    # Write video with predicted coordinates (shuttlecock tracking only)
    print("Generating video with shuttlecock tracking...")
    write_pred_video(video_file, pred_dict, save_file=out_video_file, traj_len=args.traj_len)
    print(f"Shuttlecock tracking video saved to: {out_video_file}")

    # Step 2: Player tracking using the shuttlecock-tracked video
    print("Starting player detection and tracking...")
    bytetrack_input_frames = read_video(out_video_file)
    player_tracker = PlayerTracker(model_path="ckpts/yolov11s_player_best.pt")
    player_detect = player_tracker.detect_player(bytetrack_input_frames, last_detect=False, path_of_last_detect="yolo_last_detect/list_player_dict.pkl")
    player_bbox = player_tracker.draw_player_bbox(bytetrack_input_frames, player_detect)
    
    # Save the combined video (shuttlecock + player tracking)
    print("Generating output video with shuttlecock and player tracking...")
    save_video(player_bbox, out_video_file, out_combined_video_file)
    print(f"Player-shuttlecock tracking video saved to: {out_combined_video_file}")
